...
export async function updateAWBs(ctx: any, next: () => Promise<string>) {
  try {
    ...
    // @ts-ignore
    const reverseSavedCourier = (savedCarriers && savedCarriers.length && savedCarriers[0]?.couriers)
      ? Object.assign({}, ...Object.entries(savedCarriers[0]?.couriers).map(([a, b]) => ({[b]: a,})))
      : {}
